...
- **`/rest/api/3/issue/picker` sem `currentJQL` devolve só o histórico.** A resposta tem duas seções: `hs` (o que a pessoa já abriu) e `cs` (a busca de verdade) — e a `cs` *"is only retrieved when the currentJQL parameter is specified"*. Sem o parâmetro, um seletor de item só encontra o que já foi visitado, e **quem constrói nunca vê o defeito**, porque testa com o item que acabou de abrir. Mandar JQL amplo escrito (`order by lastViewed DESC`), nunca vazio: `currentJQL` vazio para de funcionar em instância com *"Disable empty JQL queries"* ligado
- **`useTranslation()` do `@forge/react` devolve `locale`, não só `t`.** É a fonte certa para `DatePicker`/`TimePicker` (prop `locale`) e para `toLocaleDateString`. Sem isso a tela mistura o idioma do Jira (textos) com o do navegador (datas) — e num campo de data `9/2` é 2 de setembro ou 9 de fevereiro conforme o idioma
...
- **Regras do r/jira** (lidas em 28/08): anúncio, pesquisa de mercado e **pedido de feedback** só no **"Product Self-promotion megathread"** fixado (`/r/jira/comments/1s0oeox/`); post técnico próprio é permitido **se não linkar produto**; *"No AIslop"*; e a regra "Recruitment is allowed" é sobre **vaga de emprego** e proíbe *"recruit unpaid work"* — não serve para pedir testadores. Sem histórico de contribuição, a punição é **banimento, não aviso**
